...
import { AgentPaysClient } from "./agentpays-client.js";

// ─── Initialize Client ──────────────────────────────────────

let client: AgentPaysClient;

try {
  client = AgentPaysClient.fromEnv();
} catch (err) {
  console.error(
    "AgentPays MCP: Failed to initialize.\n" +
...
const TOOLS: Tool[] = [
  {
    ...
    description:
      "Send a crypto payment on-chain (USDC, EURC, ETH, etc.). " +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        chain: {
          type: "string",
          description:
            "Chain identifier: eth-sepolia, base-sep, ethereum, base, etc.",
        },
        action: {
          type: "string",
          enum: ["SEND", "PAY_API", "SWAP", "MINT", "DEPOSIT"],
          description:
            "Purpose of the payment. SEND = transfer to address, PAY_API = pay for an API call",
        },
        toAddress: {
          type: "string",
        ...
        idempotencyKey: {
          type: "string",
          description:
            "Optional unique key to prevent duplicate payments on retry",
        },
      },
  ...
  },
  {
    ...
    description:
      "Check if a payment would be approved WITHOUT actually sending money. " +
  ...
  },
  {
    ...
    description:
      "Get the agent's current status: ACTIVE, PAUSED, or REVOKED. " +
  ...
  },
  {
    ...
    description:
      "Get the agent's spending policy — daily/weekly limits per currency, " +
  ...
  },
  {
    ...
    description:
      "Get wallet balances across all assigned chains and currencies. " +
  ...
  },
  {
    ...
    description:
      "Look up a specific transaction by its AgentPays transaction ID. " +
  ...
  },
  {
    ...
    description:
      "List all wallets assigned to this agent, including chain, address, and currencies.",
    inputSchema: {
      type: "object" as const,
      properties: {},
    },
...
async function handleTool(
  ...
  switch (name) {
    ...
    case "agentpays_get_transaction": {
      const tx = await client.getTransaction(args.transactionId as string);
